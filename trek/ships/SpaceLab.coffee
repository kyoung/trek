...
class SpaceLab extends BaseShip
    ...
    constructor: ( @name, @serial="" ) ->
        ...
        if @name?
            SpaceLab.launch_ship @name
        else
            # Choose a random name to go with
            { name, registry } = do SpaceLab.get_ship_name
            @name = name
            @serial = registry
