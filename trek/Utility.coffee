...
exports.distance = ( p1, p2, round=true ) ->
    ...
    if p1.x == p2.x and p1.y == p2.y and p1.z == p2.z
        return 0
    d = Math.sqrt(Math.pow((p1.x - p2.x),2) + Math.pow((p1.y - p2.y),2) + Math.pow((p1.z - p2.z),2))
    if round
        return Math.round(d)
    else
        return d
...
exports.intercept = ( you, them, { impulse, warp } ) ->
    ...
    velocity = exports.warp_speed( warp ) + ( impulse * Constants.IMPULSE_SPEED )
    ...
    a = Math.pow(them.velocity.x, 2) + Math.pow(them.velocity.y, 2) + Math.pow(them.velocity.z, 2) - Math.pow(velocity, 2)
    b = 2 * (them.velocity.x * delta_s.x + them.velocity.y * delta_s.y + them.velocity.z * delta_s.z)
    c = Math.pow(delta_s.x, 2) + Math.pow(delta_s.y, 2) + Math.pow(delta_s.z, 2)
    p = -b / (2*a)
    q = Math.sqrt((b * b) - 4 * a * c) / (2 * a)
    t1 = p - q
    t2 = p + q
    t = switch
        when t1 > t2 and t2 > 0 then t2
        else t1
    i =
    ...
    r =
        bearing: exports.bearing(you, {position: i}, 9)
        time: t
        final_position: i
...
exports.intercept_distance = ( you, them ) ->
    ...
    distance = Math.round(Math.sqrt(Math.pow(x_intercept, 2) + Math.pow(y_intercept, 2))) + 1
