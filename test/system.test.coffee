...
exports.SystemTest =
    ...
    'test Shield systems': ( test ) ->
        ...
        # Actually... there's some tactical considerations here.
        # At a close range, we shouldn't be firing full-yield torpedoes, but
        # lower-yield ones so that we don't suffer the blast damage

        torp_level = 3  # /16
        mid_yield = Math.pow( 2, torp_level ) / Math.pow( 2, 16 )
        torpedo_blast = Torpedo.MAX_DAMAGE / Math.pow( 0.5, 2 ) * mid_yield

        for i in [0...4]
            s.hit torpedo_blast
            test.ok s.charge > 0, "#{s.name} failed to withstand #{ i } blasts: #{ s.charge }"

        # so then, a full spread of pow 3 torpedoes shouldn't know out a shield,
        # but a fifth one should
